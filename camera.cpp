...
void Camera::DrawCascadeShadowMap(int i)
{
	...
	// 定数バッファに格納
	m_LVP[i] = light.ViewMatrix * light.ProjectionMatrix;
	if (i == 2) Renderer::SetLightViewProjectionCropMatrix(m_LVP);
}
...
void Camera::ActTitleSceneCamera()
{
	...
	default:
		...
		m_Position = m_Target + m_FixedDistance;
		break;
	}
