...
void Model::Draw(MATERIAL Material)
{
	...
	{
		// マテリアル設定
		m_SubsetArray[i].Material.Material.Shininess = Material.Shininess;
		m_SubsetArray[i].Material.Material.Metalic = Material.Metalic;
		m_SubsetArray[i].Material.Material.Roughness = Material.Roughness;
		Renderer::SetMaterial(m_SubsetArray[i].Material.Material);
...
void Model::DrawInstancedIndirect(int instanceNum, ID3D11Buffer* indirectArgsBuffer)
{
	// 頂点バッファ設定
	UINT stride = sizeof(VERTEX_3D);
	UINT offset = 0;
	...
	for (unsigned int i = 0; i < m_SubsetNum; i++)
	{
		...
		indirectArgs.IndexCountPerInstance = m_SubsetArray[i].IndexNum;
		indirectArgs.InstanceCount = instanceNum;
		indirectArgs.StartIndexLocation = m_SubsetArray[i].StartIndex;
		indirectArgs.BaseVertexLocation = 0;
		indirectArgs.StartInstanceLocation = 0;

		Renderer::GetDeviceContext()->UpdateSubresource(indirectArgsBuffer, 0, nullptr, &indirectArgs, 0, 0);



		// マテリアル設定
		m_SubsetArray[i].Material.Material.Metalic = 0.5f;
		m_SubsetArray[i].Material.Material.Roughness = 0.5f;
		Renderer::SetMaterial(m_SubsetArray[i].Material.Material);
		...
		// ポリゴン描画
		Renderer::GetDeviceContext()->DrawIndexedInstancedIndirect(indirectArgsBuffer, 0);
	}
}
...
void Model::Load(const char* FileName, bool use)
{
	m_UseModelTexture = false;
	Load(FileName);
}
...
void Model::LoadObj( const char *FileName, MODEL *Model )
{
	...
	while( true )
	{
		...
		else if( strcmp( str, "f" ) == 0 )
		{
			...
			//接ベクトルと従法線の作成
			for (unsigned int t = (vc - in); t < vc; t++)
			{
				CalcTangentAndBinormal(
					&Model->VertexArray[vc - 3].Position, &Model->VertexArray[vc - 3].TexCoord,
					&Model->VertexArray[vc - 2].Position, &Model->VertexArray[vc - 2].TexCoord,
					&Model->VertexArray[vc - 1].Position, &Model->VertexArray[vc - 1].TexCoord,
					&Model->VertexArray[t].Tangent,
					&Model->VertexArray[t].Binormal);
			}
...
void Model::LoadMaterial( const char *FileName, MODEL_MATERIAL **MaterialArray, unsigned int *MaterialNum )
{
	...
	while( true )
	{
		...
		{
			//Metalic
			fscanf(file, "%f", &materialArray[mc].Material.Metalic);
		}
...
void Model::CalcTangentAndBinormal(D3DXVECTOR3* p0, D3DXVECTOR2* uv0,D3DXVECTOR3* p1, D3DXVECTOR2* uv1,D3DXVECTOR3* p2, D3DXVECTOR2* uv2,D3DXVECTOR3* outTangent, D3DXVECTOR3* outBinormal)
{
	D3DXVECTOR3 CP0[3] = {
