...
void PostProcess::Update()
{
	...
	if (scene->GetSceneName() != GAME_SCENE) return;


	// フォグ処理
	if (playerPos.x > 40.5f && playerPos.x < 116.0f
		&& playerPos.z > 62.5f && playerPos.z < 190.0f)
	{
		GUI::fog[0] = true;
		GUI::fog[1] = true;
		GUI::fogParameter.HeightColor = D3DXVECTOR4(0.65f, 0.65f, 0.65f, 1.0f);
		if (GUI::fogParameter.HeightLevel.x < 7.5f)GUI::fogParameter.HeightLevel.x += 0.04f;
	}
	else if (GUI::fogParameter.HeightLevel.x > 0.0f)
	{
		GUI::fogParameter.HeightLevel.x -= 0.04f;
	}
}
...
void PostProcess::DrawDepthOfField()
{
	...
	if (GUI::depthOfField && GUI::textureType >= 5)
	{
		...
		// テクスチャ設定
		ID3D11ShaderResourceView* texture = Renderer::GetAntiAliasingTexture();
