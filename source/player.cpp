	...
	else
	{
		...
		if (m_PlayerAnim.isAttack)
		{
			...
			else if (m_PlayerAnim.PlayerAttack == PLAYER_DASH_ATTACK)
			{
				...
				if (Input::GetKeyExit('L'))
				{
					...
					// サウンド
					if (m_Param.dashLevel <= 20)Resource::GetChargeLightSEAudio()->Play();
					else Resource::GetChargeHeavySEAudio()->Play();
				}
				// 攻撃中
		...
		else
		{
			...
			if (Input::GetKeyTrigger('K') && m_Param.canAttack)
			{
				...
				// サウンド
                Resource::GetChargeLightSEAudio()->Play();
			...
			// ジャンプ処理
			{
				...
				if (m_PlayerAnim.isJump)
				{
					...
					// サウンド
				    if (m_PlayerAnim.JumpFrame == 6) Resource::GetJumpSEAudio()->Play();
			...
			{   // 回避中の場合
				if (m_PlayerAnim.isDodge)
				{
					...
					// サウンド
					if (m_PlayerAnim.DodgeFrame == 1) Resource::GetDodgeSEAudio()->Play();
...
void Player::SetHealHP(float heal)
{
	...
	PlayerUI::ResetStatusUICount();
	// サウンド
    Resource::GetHealSEAudio()->Play();
}

void Player::Respawn()
{
	// 座標・ステータスを初期化
	m_Position = m_RespwanPosition + D3DXVECTOR3(0.0f, 7.0f, 00.0f);
