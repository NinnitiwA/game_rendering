...
void GUI::ImGuiRenderRenderingMenu()
{
	...
	if (ImGui::CollapsingHeader("PostProcess"))
	{
		...
		if (postProcessFlag)
		{
			...
			// フォグ
			static bool fogFlag[3] = { false }; // フォグ使用識別子、高度フォグ、深度フォグ
			fog = fogFlag[0];
			ImGui::CheckboxNameless("Fog", &fogFlag[0]);
			ImGui::SameLine();
			if (ImGui::TreeNode("Fog"))
			{
				ImGui::Checkbox("Height Fog", &fogFlag[1]);
				...
				ImGui::Checkbox("Depth Fog", &fogFlag[2]);
				if (fogParameter.DepthColor.w > 0.5f)
				{
					ImGui::ColorEdit4("", (float*)&fogParameter.DepthColor);
					ImGui::SliderFloat("CenterZ:%.4f", &fogParameter.DepthLevel.x, -1000.0f, 1000.0f);
					ImGui::SliderFloat("Width:%.4f", &fogParameter.DepthLevel.y, -1000.0f, 1000.0f);

				}

				// 定数バッファ用フォグ識別子
				if (fogFlag[1]) fogParameter.HeightColor.w = 1.0f;
				else fogParameter.HeightColor.w = 0.0f;
				if (fogFlag[2]) fogParameter.DepthColor.w = 1.0f;
...
void GUI::ImGuiRenderSRV()
{
	...
	const char* items[] = { "G-Buffer", "Reflection", "ShadowMap", "DOF", "Fog", "Luminance", "Bloom", "AA"};
